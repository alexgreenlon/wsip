...
all.afe.16s<-read.csv("/Users/alexgreenlon/soil-C-SFA/wsip/results/16S-comparison/16S-vs-contig-vs-bin-afe.tsv",header=T,sep="\t")
mydat<-read.csv("soil-C-SFA/docs/wsip-metawrap-bins.genome-info.10.7.20.csv",header=T,sep='\t')
afe.16s<-merge(afe.16s,mydat,by.x="bin",by.y="me")
...
coef(lm(X16S.afe.median~bin.afe.median,data=afe.16s))
cor.test(afe.16s$X16S.afe.median,afe.16s$bin.afe.median)

fig.4<-ggplot(afe.16s,aes(x=bin.afe.median,y=X16S.afe.median,shape=site,color=phylum))+geom_point() + scale_color_manual(values=mycols) +
  ...
  geom_hline(yintercept=0) + geom_abline(intercept=0.04995637,slope=0.52794594) + geom_vline(xintercept=0) +
  xlab("Genome AFE") +  ylab("16S AFE")

coef(lm(X16S.afe.median~AFE,data=afe.16s))
cor.test(afe.16s$X16S.afe.median,afe.16s$AFE)

fig.4c<-ggplot(all.afe.16s,aes(x=AFE,y=X16S.afe.median))+geom_point() +
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),legend.position = "none") +
  geom_hline(yintercept=0) + geom_abline(intercept=-0.009113378,slope=0.819933926) + geom_vline(xintercept=0) +
  xlab("Contig AFE") +  ylab("16S AFE")
